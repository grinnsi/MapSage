...
@router.get(
    ...
    responses={
        200: {"model": FeatureGeoJSON, "description": markdown.markdown("fetch the feature with id `featureId` in the feature collection with id `collectionId`")},
        404: {"description": markdown.markdown("The requested resource does not exist on the server. For example, a path parameter had an incorrect value.")},
        500: {"model": Exception, "description": markdown.markdown("A server error occurred.")},
    },
    tags=["Data"],
...
async def get_features(
    ...
    if not BaseDataApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseDataApi.subclasses[0]().get_features(collectionId, limit, bbox, datetime)
